...
def get_batch_analytics(
    ...
    if not logs:
        return {"message": "No data available for this batch in the specified date range."}
    ...
    for metric in metrics_list:
        ...
        values = [getattr(log, metric) for log in logs if getattr(log, metric) is not None]
...
def get_batch_summary(batch_id: UUID, db: Session = Depends(get_db)):
    ...
    * **Moisture:** 75.0% to 90.0%
    ...
    logs = db.query(DailyLog).filter(DailyLog.batch_id == batch_id).order_by(DailyLog.log_date.asc()).all()
