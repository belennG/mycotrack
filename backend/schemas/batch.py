...
class BatchBase(BaseModel):
    ...
    batch_name: str = Field(..., min_index=1, max_length=100, description="Unique identifier for the batch")
    crop_type: str = Field(..., min_length=1, max_length=100, description="Species or type of crop")
    status: BatchStatus = Field(default=BatchStatus.ACTIVE, description="Current lifecycle status")
...
class BatchCreate(BatchBase):
    ...
    start_date: Optional[datetime] = Field(default_factory=lambda: datetime.now(timezone.utc), description="Start date of the batch")
